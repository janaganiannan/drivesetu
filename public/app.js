...
var applications = [
    ...
    { id: 'APP-206500', name: 'Rahul Sharma', type: 'Permanent Licence', status: 'Submitted', date: '15 Aug 2026', citizenId: 'citizen@drivesetu.com' },
...
function allocateIndependentOfficer(testRtoCode, excludeOfficerIds) {
    ...
    var eligible = rtoAccounts.filter(function(acc) {
        ...
        // MUST NOT belong to the same RTO where physical test was conducted!
        if (acc.rtoCode === testRtoCode) return false;
        // MUST NOT be in exclude list (e.g. previous reviewer)
    ...
    if (eligible.length === 0) {
        ...
        return fallback[0] || rtoAccounts[7]; // Officer 17 as default
    }

    // Default to Officer 17 (TG-08) for TG-03 test centre to demonstrate TG-03 -> TG-08 cross-RTO allocation
    ...
    // If Officer 17 rejected, default to Officer 31 (TG-12) for second review
...
function adminEscalateForReview(appId) {
    var session = sessionStorage.getItem('rtoSession') ? JSON.parse(sessionStorage.getItem('rtoSession')) : null;
    if (!session || session.role !== 'ADMIN') { alert('⛔ Only System Administrators can escalate.'); return; }
    var reason = prompt('Enter reason for governance review escalation.\n\nIMPORTANT: This does NOT change the application decision.');
    if (!reason || reason.trim() === '') return;
    appendAuditEvent(appId, 'ESCALATION_REQUESTED', session.name, 'ADMIN', 'Governance Review Escalation: ' + reason);
    var apps = getStoredApplications();
    ...
    saveStoredApplications(apps);
    alert('📋 Escalation recorded in audit log.\n\nThis does NOT change the application decision.\nGovernance review has been flagged.');
    render();
}
...
function renderAdminDashboard() {
    ...
    var appRows = allApps.map(function(app) {
        ...
        var escalateBtn = '<button class="btn btn-ghost" style="padding:0.25rem 0.6rem; font-size:0.72rem; color:#7c3aed; border:1px solid #c4b5fd;" onclick="adminEscalateForReview(\'' + app.id + '\')"><i class="fa-solid fa-flag"></i> Escalate</button>';
...
function render() {
    ...
    if (isHome) {
        ...
        if (!_rs || (_rs.role !== 'REVIEWING_OFFICER' && _rs.role !== 'ADMIN')) {
            ...
            // Filter applications assigned to this reviewer officer
            allApps = allApps.filter(function(a) {
                return (a.evaluator1 && a.evaluator1.officerId === _rs.officerId) ||
                    ...
                    '<div class="card" style="padding:1rem;">' +
                        ...
                        '<div style="padding:0.75rem; background:#f4fbf8; border:1px solid #c2ead8; border-radius:var(--radius-md);">' +
                            '<div class="flex-between" style="margin-bottom:0.4rem;">' +
                                ...
                                '<span class="badge badge-approved">Decision Support</span>' +
                            '</div>' +
                            '<div style="margin-top:0.5rem; border-top:1px dashed #c2ead8; padding-top:0.5rem;">' +
                                (pdfDataSrc
                                    ...
                                    : '<div style="background:#fff; border:1px solid #c2ead8; border-radius:6px; padding:1.25rem 1rem; text-align:center;">' +
                                        ...
                                        '<div style="font-size:0.78rem; color:var(--text-muted); margin-top:0.25rem;">Prototype AI Analysis Report — Telemetry Score: <strong>95/100 (PASSED)</strong>.</div>' +
                                      '</div>'
                                ) +
...
function openVideoDocument(appId) {
    ...
    var vUrl = (item && item.videoDataUrl) ? item.videoDataUrl : (window.lastUploadedVideoURL || 'https://www.w3schools.com/html/mov_bbb.mp4');
...
function viewLearnerLicenceDoc(appId) {
    ...
    var name = app ? app.name : 'Applicant';
    var sd = (app && app.serviceDetails) ? app.serviceDetails : {};
    var rto = sd.rtoOfficeName || 'RTO Authority';
    var date = app ? app.date : new Date().toLocaleDateString('en-IN');
    var categories = (sd.vehicleClasses && sd.vehicleClasses.length > 0) ? sd.vehicleClasses.join(', ') : 'MCWG, LMV';

    var docHTML = '<!DOCTYPE html><html><head><title>Learner\'s Licence - ' + (appId || 'LL') + '</title>' +
        '<meta charset="utf-8"/>' +
        '<style>' +
        'body { font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif; background: #f0f4f2; margin: 0; padding: 2rem; color: #1e293b; }' +
        '.ll-container { background: #ffffff; max-width: 700px; margin: 0 auto; padding: 2.5rem; border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.12); border: 2px solid #1aab74; }' +
        ...
        '.badge { background: #e8f7f1; color: #1aab74; padding: 0.3rem 0.8rem; border-radius: 20px; font-weight: 700; font-size: 0.8rem; border: 1px solid #c2ead8; }' +
        ...
        '.note { font-size: 0.8rem; color: #475569; background: #fff7e6; border: 1px solid #ffe7ba; padding: 0.8rem; border-radius: 6px; margin-top: 1rem; }' +
        '</style></head><body>' +
        '<div class="ll-container">' +
            '<div class="header">' +
                '<div><h1 class="title">🪪 FORM 3 - LEARNER\'S LICENCE</h1><div class="subtitle">Government of India • Regional Transport Office (' + rto + ')</div></div>' +
                '<span class="badge">OFFICIAL LEARNER\'S LICENCE</span>' +
            '</div>' +
            '<div class="section grid">' +
                '<div><div class="field-label">Licence Application ID</div><div class="field-val">' + (appId || 'APP-101') + '</div></div>' +
                '<div><div class="field-label">Licence Type</div><div class="field-val">Learner\'s Licence (LLR)</div></div>' +
                '<div><div class="field-label">Name of Holder</div><div class="field-val">' + name + '</div></div>' +
                '<div><div class="field-label">Issue Date</div><div class="field-val">' + date + '</div></div>' +
                '<div><div class="field-label">Allowed Vehicle Categories</div><div class="field-val">' + categories + '</div></div>' +
                '<div><div class="field-label">Validity</div><div class="field-val">6 Months from Issue Date</div></div>' +
            '</div>' +
            '<div class="note">' +
                '<strong>Important Notice for Learner\'s Licence Holder:</strong><br>' +
                '• The holder is permitted to drive as a learner only when accompanied by an instructor holding a valid Permanent Driving Licence.<br>' +
                '• "L" plate must be prominently displayed on the front and rear of the vehicle.<br>' +
                '• You are eligible to apply for a Permanent Driving Licence after 30 days from the issue date.' +
            '</div>' +
            '<p style="font-size:0.78rem; color:#94a3b8; text-align:center; margin-top:2rem;">This is a computer-generated Learner\'s Licence document issued by DriveSetu RTO Portal.</p>' +
        '</div>' +
        '</body></html>';
    ...
    if (win) {
        ...
        alert('Please allow popups to view the Learner\'s Licence document.');
    }
}
...
function viewPdfDocument(appId) {
    ...
    var pdfDataSrc = (item && item.pdfDataUrl) ? item.pdfDataUrl : (window.lastUploadedPdfURL || null);

    if (pdfDataSrc && pdfDataSrc.startsWith('data:application/pdf;base64,')) {
        // Open Base64 PDFs inside a new tab cleanly
        var win = window.open();
        if (win) {
            win.document.write('<iframe src="' + pdfDataSrc + '" style="width:100%; height:100%; border:none;"></iframe>');
            win.document.close();
            return;
        }
    }
...
function submitServiceForm(licenceType) {
    ...
    var newApp = {
        ...
        assignedOfficer: 'Officer ' + (1 + Math.floor(Math.random() * 5)),
        reviewHistory: [],
        remarks: '',
