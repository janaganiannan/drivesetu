-- ====================================================================
-- DRIVESETU COMPLETE DATABASE ARCHITECTURE & STORAGE POLICIES
...
REVOKE ALL ON TABLE public.rto_info FROM anon, public;
...
CREATE POLICY "Service role full access on rto_info" ON public.rto_info FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Authenticated RTO officers can view rto_info" ON public.rto_info FOR SELECT TO authenticated
    USING (auth.jwt() ->> 'email' = officer_email OR (auth.jwt() -> 'user_metadata' ->> 'role') IN ('RTO_OFFICER', 'SUPER_ADMIN', 'ADMIN'));
...
('RTA Telangana (Medchal)', 'TG-03', 'Regional Transport Office', 'Telangana', 'Medchal-Malkajgiri', 'RTO Office Premises, Medchal', '500043', '040-23456789', 'annanjanagani003@gmail.com', 'Annan Janagani', 'OFF-TG03-01', 'RTO Reviewing Officer', '8125531017', 'annanjanagani003@gmail.com'),
('RTA Secunderabad', 'TG-05', 'Regional Transport Office', 'Telangana', 'Hyderabad', 'RTO Office Premises, Secunderabad', '500003', '040-23456780', 'employ1@drivesetu.com', 'Employ 1', 'EMP-001', 'RTO Evaluator', '9876543210', 'employ1@drivesetu.com') 
ON CONFLICT (rto_code) DO UPDATE SET officer_email = EXCLUDED.officer_email, officer_full_name = EXCLUDED.officer_full_name;
...
REVOKE ALL ON TABLE public.citizen_info FROM anon, public;
...
CREATE POLICY "Service role full access on citizen_info" ON public.citizen_info FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Citizens can view own profile" ON public.citizen_info FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Citizens can update own profile" ON public.citizen_info FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Citizens can insert own profile" ON public.citizen_info FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
...
REVOKE ALL ON TABLE public.citizen_documents FROM anon, public;
...
CREATE POLICY "Service role full access on citizen_documents" ON public.citizen_documents FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "Citizens can view own documents" ON public.citizen_documents FOR SELECT TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Citizens can update own documents" ON public.citizen_documents FOR UPDATE TO authenticated USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Citizens can insert own documents" ON public.citizen_documents FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
...
-- ====================================================================
-- 6. SUPABASE STORAGE BUCKET & POLICIES (citizen-documents)
-- ====================================================================
INSERT INTO storage.buckets (id, name, public) 
VALUES ('citizen-documents', 'citizen-documents', true)
ON CONFLICT (id) DO UPDATE SET public = true;

DROP POLICY IF EXISTS "Public access for citizen-documents" ON storage.objects;
DROP POLICY IF EXISTS "Authenticated users upload to citizen-documents" ON storage.objects;
DROP POLICY IF EXISTS "Authenticated users update citizen-documents" ON storage.objects;

CREATE POLICY "Public access for citizen-documents"
    ON storage.objects FOR SELECT TO public USING (bucket_id = 'citizen-documents');

CREATE POLICY "Authenticated users upload to citizen-documents"
    ON storage.objects FOR INSERT TO public WITH CHECK (bucket_id = 'citizen-documents');

CREATE POLICY "Authenticated users update citizen-documents"
    ON storage.objects FOR UPDATE TO public USING (bucket_id = 'citizen-documents');
